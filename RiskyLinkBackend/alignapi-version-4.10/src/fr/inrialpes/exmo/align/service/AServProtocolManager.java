 ...
 *
 * Copyright (C) INRIA, 2006-2015, 2017, 2020
 *
...
public class AServProtocolManager implements Service {
   ...
   /*********************************************************************
	...
	try ( PrintWriter writer = new PrintWriter (
					new BufferedWriter(
					     new OutputStreamWriter( result, StandardCharsets.UTF_8 )), true) ) {
	...
	} catch ( Exception ex ) { // These are exceptions related to I/O
	    //logger.trace( "Resulting rendering : {}", resultString );
	    logger.error( "Cannot render alignment", ex );
	    return new Message( params, newId(), serverId,"Failed to render alignment" );
	}
	
	try {
	    resultString = result.toString( StandardCharsets.UTF_8.name() );
	} catch ( UnsupportedEncodingException ueex ) {
	    logger.debug( "IGNORED Exception", ueex );
	}	    
	return new RenderedAlignment( params, newId(), serverId, resultString );
    }
	...
	al = new BasicAlignment( (BasicAlignment)al );
	...
	try ( PrintWriter writer = new PrintWriter (
				      new BufferedWriter(
					  new OutputStreamWriter( result, StandardCharsets.UTF_8.name() )), true) ) {
	    try {
		noo.write( writer );
	    } catch (AlignmentException alex) {
	...
	return new RenderedNetwork( params, newId(), serverId, resultString );
    }
	...
	if ( pidON != null ) result = "<h1>"+id+" ("+pidON+")"+"</h1>";
	else result = "<h1>"+id+"</h1>";
	...
	noo = new BasicOntologyNetwork( noo );
	noo.setExtension( Namespace.EXT.uri, Annotations.PRETTY, pretty );
	// match
    	try { 
	...
	while( enumeration != null && enumeration.hasMoreElements() ){
		...
		if( canon.endsWith(".jar") &&
		    !entry.isDirectory() && // Some put directory named .jar!
	...
	try {
	    ...
	    if ( classname.equals( "module-info" ) // This thing in some jars is not a class
		 || classname.startsWith( "com.mysql" ) // avoids loading deprecated Driver
		 ...
		 || classname.startsWith( "org.apache.lucene" )
		 ...
		 || classname.startsWith( "jade." ) // jade.tools.rma.StartDialog raises problems
		 ) return false;
