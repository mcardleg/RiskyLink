 ...
 * $Id$
 *
 * Copyright (C) INRIA, 2003-2004, 2007-2018
 *
...
public class OWLAxiomsRendererVisitor extends IndentedRendererVisitor implements AlignmentVisitor, EDOALVisitor {
	...
	if ( align instanceof ObjectAlignment ) {
	    ...
	    try {
		alignment = new ObjectAlignment( (URIAlignment)align );
		onto1 = ((ObjectAlignment)alignment).getOntologyObject1();
		onto2 = ((ObjectAlignment)alignment).getOntologyObject2();
     ...
     * 
     * The trick will thus be to create named properties and relations
     * that can be used for correspondences
     * For that purpose, we need to first collect all the created names and their constraints and then generate the correspoding axioms:
     * and --> rdfs:subPropertyOf
     * or --> ?
     * comp --> ? create the composition
     * Property/RelationValueRestriction --> rdfs:domain
     * Property/RelationDomainRestriction --> rdfs:domain
     * Property/RelationType/CoDomainRestriction --> rdfs:range
     */
	...
	case EQUAL: cardinality = "cardinality"; break;
	case LOWER: cardinality = "maxCardinality"; break;
	case GREATER: cardinality = "minCardinality"; break;
	default: throw new AlignmentException( "Unknown comparator : "+c.getComparator().getURI() );
	}
