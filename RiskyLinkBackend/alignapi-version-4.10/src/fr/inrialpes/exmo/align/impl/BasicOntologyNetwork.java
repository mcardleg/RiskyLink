 ...
 * $Id$
 *
 * Copyright (C) INRIA, 2009-2010, 2014-2015, 2017
 *
...
public class BasicOntologyNetwork implements OntologyNetwork, Extensible {
	...
	for ( URI onto : noo.ontologies.keySet() ) addOntology( onto );
	for ( Alignment al : noo.alignments ) {
	    try { 
		addAlignment( al );
	...
	setExtensions( noo.extensions.convertExtension( "cloned", this.getClass().getName()+"#clone" ) );
    }
    ...
    */
	...
	try {
	    network = (BasicOntologyNetwork)super.clone();
	} catch( CloneNotSupportedException cnsex ) {
	    logger.debug( "IGNORED Exception: network not cloned", cnsex );
	    return null;
	}
	...
	if ( als.size() == 0 ) { // If no element, create new
	    ...
	    result = new BasicAlignment( (BasicAlignment)als.iterator().next() );
	}
	return result;
    }
	...
	if ( als.size() == 0 ) { // If no element, create new
	    ...
	    result = new BasicAlignment( (BasicAlignment)it.next() );
	    while ( it.hasNext() ) {
		result.join( it.next() );
	    }
	}
	...
	} finally { // Clean up memory (not very good to close where not openned)
	    rdfModel.close();
	}
	return on;
    }
