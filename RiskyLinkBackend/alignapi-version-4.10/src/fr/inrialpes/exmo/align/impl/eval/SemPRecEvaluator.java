 ...
 * $Id$
 *
 * Copyright (C) INRIA, 2009-2014, 2017, 2020
 *
...
public class SemPRecEvaluator extends PRecEvaluator implements Evaluator {
    ...
    // null means that we use the reduced semantics with HermiT, otherwise, this is an DRAON semantics
	...
	if ( semantics != null ) { // DRAON
	    ArrayList<Alignment> allist = new ArrayList<Alignment>();
	    allist.add( al1 );
	    try {
		reasoner = new DRAON( allist, semantics );
	    } catch ( Exception idex ) { // DRAONException... useful when no DRAON
		throw new AlignmentException( "Cannot create DRAON", idex );
	    }
	} else { // Hermit
	...
	for ( Cell c2 : al2 ) {
	    ...
	    if ( semantics != null ) { // DRAON
		try {
		    if ( ((DRAON)reasoner).isEntailed( al2, c2 ) ) { 
	    ...
	    Thread myThread = new Thread(
		  new Runnable(){
			  ...
			  try ( PrintWriter writer = new PrintWriter (
					     new BufferedWriter(
								new OutputStreamWriter( out, StandardCharsets.UTF_8.name() )), true) ) {
	...
	try {
	    ...
	    merged.deleteOnExit();
	    try ( PrintWriter writer = new PrintWriter ( merged, StandardCharsets.UTF_8.name() ) ) {
		OWLAxiomsRendererVisitor renderer = new OWLAxiomsRendererVisitor(writer);
		renderer.init( new Properties() );
		align.render( renderer );
	    } catch (UnsupportedEncodingException ueex) {
	    ...
	    throw new AlignmentException( "Error writing in temporary file", ioex );
	}
