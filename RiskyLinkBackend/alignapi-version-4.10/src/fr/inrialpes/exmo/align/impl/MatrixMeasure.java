 ...
 * $Id$
 *
 * Copyright (C) INRIA, 2003-2010, 2013-2014, 2017-2018
 *
...
public abstract class MatrixMeasure implements Similarity {
	...
	try {
		...
		oalign = new ObjectAlignment( (URIAlignment)align );
	...
	try {
		...
		classlist2.put( cl, Integer.valueOf(nbclass2++) );
	    }
	    for( Object cl : onto1.getClasses() ){
		classlist1.put( cl, Integer.valueOf(nbclass1++) );
	    }
		...
		proplist2.put( pr, Integer.valueOf(nbprop2++) );
	    }
	    for( Object pr : onto2.getDataProperties() ){
		proplist2.put( pr, Integer.valueOf(nbprop2++) );
	    }
	    for( Object pr : onto1.getObjectProperties() ){
		proplist1.put( pr, Integer.valueOf(nbprop1++) );
	    }
		...
		if ( onto2.getEntityURI( ind ) != null ) {
		    indlist2.put( ind, Integer.valueOf(nbind2++) );
		}
	    }
	    for( Object ind : onto1.getIndividuals() ){
		// We suppress anonymous individuals... this is not legitimate
		if ( onto1.getEntityURI( ind ) != null ) {
		    indlist1.put( ind, Integer.valueOf(nbind1++) );
		}
	    }
