 ...
 * $Id$
 *
 * Copyright (C) INRIA, 2003-2011, 2013-2017, 2020
...
public class BasicAlignment implements Alignment, Extensible {
	...
	this(); // in principle super(al);
	try { init( al.getOntologyObject1(), al.getOntologyObject2(), al.getRelationType(), al.getConfidenceType() );
	...
	try {
		...
		logger.error( "Relation class not found: {}", rel, cnfex );
		throw new AlignmentException ( "Relation class not found", cnfex );
	    } catch ( NoSuchMethodException nsmex ) {
		logger.error( "Cannot create relation for {}", rel, nsmex );
		throw new AlignmentException ( "Cannot create relation", nsmex );
		...
		logger.debug( "IGNORED Exception: created Basic Relation)", iex );
	...
	if ( method.equals("perc") ){
	    i = (Double.valueOf(size*threshold)).intValue();
	} else if ( method.equals("best") ){
	    i = java.lang.Math.min( size, (Double.valueOf(threshold*100)).intValue() );
   ...
   /**
	...
	try {
	    align = (BasicAlignment)super.clone();
	} catch( CloneNotSupportedException cnsex ) {
	    logger.debug( "IGNORED Exception: alignment not cloned", cnsex );
	    return null; 
	}
	...
	// The only one that remain (deep cloning of extensions, not of cells)
