...
public class GenPlot extends CommonCLI {
	...
	for ( Pair npair : input ) {
	    if ( Double.compare( npair.getX(), next ) == 0 ) {
		output.add( npair );
		next += STEP;
	...
	for( Vector<Pair> table : result ) {
	    ...
	    try ( PrintWriter auxwriter = new PrintWriter (
				    new BufferedWriter(
                                       new OutputStreamWriter(
							      new FileOutputStream(new File(dir,algo+".table")), StandardCharsets.UTF_8.name() )), true) ) {
		// Print header
