...
public abstract class GraphPatternRendererVisitor extends IndentedRendererVisitor implements EDOALVisitor {
	...
	queries.put( referer, query );
        if ( split ) {
            try ( OutputStreamWriter writer = new OutputStreamWriter( new FileOutputStream(splitdir + "query" + fileIndex + ".rq"), StandardCharsets.UTF_8 ) ) {
