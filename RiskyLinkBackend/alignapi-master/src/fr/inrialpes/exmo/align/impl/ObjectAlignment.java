 ...
 * $Id$
 *
 * Copyright (C) INRIA, 2003-2011, 2013-2015, 2017
 *
...
public class ObjectAlignment extends BasicAlignment {
	...
	this();
	try {
	    init( al.getFile1(), al.getFile2(), al.getRelationType(), al.getConfidenceType() );
	} catch ( AlignmentException alex ) {
	    try { // Really a friendly fallback
		init( al.getOntology1URI(), al.getOntology2URI(), al.getRelationType(), al.getConfidenceType() );
	    } catch ( AlignmentException xx ) {
		throw alex;
	    }
	}
	setType( al.getType() );
	setLevel( al.getLevel() );
	setExtensions( al.extensions.convertExtension( "ObjectURIConverted", "fr.inrialpes.exmo.align.ObjectAlignment#toObject" ) );
	LoadedOntology<Object> o1 = getOntologyObject1();
	LoadedOntology<Object> o2 = getOntologyObject2();
	Object obj1 = null;
	Object obj2 = null;
	...
	try {
	    for ( Cell c : al ) {
		try {
		    obj1 = o1.getEntity( c.getObject1AsURI( this ) );
		} catch ( NullPointerException npe ) {
		    throw new AlignmentException( "Cannot dereference entity "+c.getObject1AsURI( this ), npe );
		}
		try {
		    obj2 = o2.getEntity( c.getObject2AsURI( this ) );
		} catch ( NullPointerException npe ) {
		    throw new AlignmentException( "Cannot dereference entity "+c.getObject2AsURI( this ), npe );
		}
		//logger.trace( "{} {} {} {}", obj1, obj2, c.getRelation(), c.getStrength() );
		if ( obj1 == null ) throw new AlignmentException( "Cannot dereference entity "+c.getObject1AsURI( this ) );
		if ( obj2 == null ) throw new AlignmentException( "Cannot dereference entity "+c.getObject2AsURI( this ) );
		Cell newc = addAlignCell( c.getId(), obj1, obj2,
	...
	this();
	try {
	    init( al.getOntologyObject1(), al.getOntologyObject2(), al.getRelationType(), al.getConfidenceType() );
	} catch ( AlignmentException alex ) {
	    logger.debug( "IGNORED Exception: alignment not cloned", alex );
	}
	setType( al.getType() );
	setLevel( al.getLevel() );
	setFile1( al.getFile1() );
	setFile2( al.getFile2() );
	for ( Entry<Object,Object> e : al.namespaces.entrySet() ) {
	    setXNamespace( (String)e.getKey(), (String)e.getValue() );
	}
	try { ingest( al ); }
	catch (AlignmentException alex) { 
	    logger.debug( "IGNORED Exception: alignment not ingested", alex );
	}
	setExtensions( al.getExtensionsObject().convertExtension( "cloned", this.getClass().getName()+"#clone" ) );
    }
	...
	if ( al instanceof URIAlignment ) {
	    init = new ObjectAlignment( (URIAlignment)al );
	} else if ( al instanceof ObjectAlignment ) {
	    init = (ObjectAlignment)al;
	} else throw new AlignmentException( "Cannot load alignment : "+al );
    }
