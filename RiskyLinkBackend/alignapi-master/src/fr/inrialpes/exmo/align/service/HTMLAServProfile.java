 ...
 * $Id$
 *
 * Copyright (C) INRIA, 2006-2015, 2017, 2021
 *
...
public class HTMLAServProfile implements AlignmentServiceProfile {
	...
	result +=
	    "<form action=\"html/\"><button style=\"background-color: lightblue;\" title=\"Alignment menu\" type=\"submit\">Alignments</button></form>"
	    + "<form action=\"noo/\"><button style=\"background-color: lightgreen;\" title=\"Network of ontologies menu\" type=\"submit\">Ontology networks</button></form>"
	    + "<form action=\"../admin/\"><button style=\"background-color: lightpink;\" title=\"Server management functions\" type=\"submit\">Server management</button></form>"
	    + "</div>"
	    ...
	    + "(C) "+AlignmentService.class.getPackage().getImplementationVendor()+"<br />"
	    + "<a href=\"https://gitlab.inria.fr/moex/alignapi\">https://gitlab.inria.fr/moex/alignapi</a><br />"
	    + "</div></center>";
	return result;
    }
     ...
     *
     * @param uri: the URL comming from the HTTP server
	...
	// Ugly and returned by catched by FindBugs
	if ( returnType == HTTPResponse.MIME_RDFXML ) {
