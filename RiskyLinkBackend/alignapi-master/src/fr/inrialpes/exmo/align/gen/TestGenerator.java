 ...
 * $Id$
 *
 * Copyright (C) INRIA, 2011-2013, 2015, 2017, 2020
 *
...
public class TestGenerator {
	...
	// JE 2020: Replaced this because FM.get() deprecated.
	...
	try ( FileOutputStream fout = new FileOutputStream( f ) ) {
	    ...
	    writer.write( model.getBaseModel(), new OutputStreamWriter(fout, StandardCharsets.UTF_8.name()), "");
        } catch ( UnsupportedEncodingException ueex ) {
	    logger.debug( "IGNORED Exception", ueex );
	} catch ( IOException ioex ) {
	    logger.error( "Cannot open file {}", destFile, ioex );
            ioex.printStackTrace();
        }
    }

    private void outputTestDirectory( OntModel onto, Alignment align, String dirName ) throws AlignmentException {
	...
	if ( !( directory.exists() && directory.isDirectory() )
	     && !directory.mkdir() ) {
	    throw new AlignmentException( "Cannot save in directory "+dir );
	}
	...
	try ( PrintWriter writer = new PrintWriter (
					 ...
					 new OutputStreamWriter(
					    new FileOutputStream( dir+"/"+alignname ), StandardCharsets.UTF_8.name() )), true) ) {
