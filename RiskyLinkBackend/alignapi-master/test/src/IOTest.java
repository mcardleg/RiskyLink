...
public class IOTest {
	...
	assertEquals( alignment.getOntology2URI().toString(), "https://moex.gitlabpages.inria.fr/alignapi/tutorial/edu.mit.visus.bibtex.owl" );
	...
	for ( Cell c : alignment ) {
	    System.err.println( ((BasicCell)c).getObject1AsURI(alignment)+" == "+StringEscapeUtils.escapeXml10(((BasicCell)c).getObject1AsURI(alignment).toASCIIString())+" == "+java.net.URLDecoder.decode(((BasicCell)c).getObject1AsURI(alignment).toString(), StandardCharsets.UTF_8.name()) );
	    System.err.println( ((BasicCell)c).getObject2AsURI(alignment)+" == "+StringEscapeUtils.escapeXml10(((BasicCell)c).getObject2AsURI(alignment).toASCIIString())+" == "+java.net.URLDecoder.decode(((BasicCell)c).getObject2AsURI(alignment).toString(), StandardCharsets.UTF_8.name()) );
	    }
	*/
	...
	// output RDF utf-8
	Alignment resutfutf = renderparse( alignment, "utfutf", StandardCharsets.UTF_8.name() );
	assertNotNull( resutfutf );
	Alignment resisoutf = renderparse( refalign, "isoutf", StandardCharsets.UTF_8.name() );
